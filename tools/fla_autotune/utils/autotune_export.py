# adapter from: https://github.com/fla-org/flash-linear-attention/blob/main/scripts/utils/autotune_export.py
...
class KernelConfigFileWriter:
    ...
    def write_backup(
        ...
        if backup_file.exists():
            try:
                ...
                if isinstance(existing_data,
                              dict) and isinstance(existing_data.get("autotune_entries"), dict):
        ...
        backup_data = {
            ...
            "autotune_entries": {
                k: existing_backup[k]
                for k in sorted(existing_backup)
            },
        }
        with open(backup_file, 'w') as f:
            json.dump(backup_data, f, indent=2)

    def save(self, output_file: Path, kernel_name: str,
             existing: KernelConfigFile | None) -> tuple[str, Path | None]:
...
def extract_configs(triton_cache_dir: Path, output_dir: Path) -> None:
    ...
    if not triton_cache_dir.exists():
        print(
            f"Triton cache directory not found: {triton_cache_dir}. Exiting as there's nothing to extract."
        )
        sys.exit(1)
