# adapter from: https://github.com/fla-org/flash-linear-attention/blob/main/scripts/utils/autotune_generate.py
...
class FLACacheGenerator:
    ...
    def _prepare_tensors(self) -> tuple:
        ...
        q, k, v, beta, h0 = map(
            lambda x: x.to(self.device).requires_grad_(True), (q, k, v, beta, h0)
        )
        ...
        for use_qk_l2norm_in_kernel in (False, True):
            tri, tri_ht = chunk_gated_delta_rule(
                q=(
                    ...
                    if not use_qk_l2norm_in_kernel else q.clone()
                ),
                k=(
                    F.normalize(k.clone(), p=2, dim=-1)
                    if not use_qk_l2norm_in_kernel else k.clone()
                ),
                v=v.clone(),
                g=g.clone(),
                beta=beta.clone(),
            ...
            ((tri * do).sum() +
             (tri_ht * dht).sum()).backward(retain_graph=not use_qk_l2norm_in_kernel)
