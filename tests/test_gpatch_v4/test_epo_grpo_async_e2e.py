"""EPO-lite GRPO async L2 e2e: feature_store history under async_rollout.

Same assertions as sync ``test_epo_grpo_e2e``, but uses ``GrpoSingleCtrlTrainer``
+ ``test_epo_grpo_async_e2e`` yaml (disaggregated, async_rollout=True).
"""
import os
import shutil
import unittest
...
_CKPT = "save_epo_grpo_async_e2e"
_DATASET_N_LINES = 32
_CONFIG_NAME = "test_epo_grpo_async_e2e"
...
class TestEpoGrpoAsyncE2e(unittest.IsolatedAsyncioTestCase):
    def tearDown(self):
        kill_all_actors_and_shutdown_ray()
    ...
    @requires_sglang
    async def test_async_history_accumulate_save_load(self):
        _prepare_dataset()
        shutil.rmtree(_CKPT, ignore_errors=True)

        config_a = load_config(_CONFIG_NAME, RlConfig)
        assert config_a.placement_type == "disaggregated"
        assert config_a.training.async_rollout
        assert config_a.training.single_controller
