...
from gpatch_v4.utils.data_manipulate_utils import divide_by_chunk_size, union_two_dict
...
def union_numpy_dict(tensor_dict1: dict[np.ndarray],
                     tensor_dict2: dict[np.ndarray]) -> dict[np.ndarray]:
...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if len(self.non_tensor_batch) != 0:
            ...
            ) == 1, "only support num_batch_dims=1 when non_tensor_batch is not empty."
    ...
    def select(
        ...
        if non_tensor_batch_keys is not None:
            non_tensor_batch = {
                key: val
                for key, val in self.non_tensor_batch.items() if key in non_tensor_batch_keys
            }
        else:
        ...
        if meta_info_keys is not None:
            sub_meta_info = {
                key: val
                for key, val in self.meta_info.items() if key in meta_info_keys
            }
        else:
    ...
    def chunk(self, chunks: int) -> List["DataProto"]:
        ...
        for key, val in self.non_tensor_batch.items():
            ...
            assert len(
                non_tensor_lst
            ) == chunks, f"len(non_tensor_lst) {len(non_tensor_lst)} != chunks {chunks}"
    ...
    def repeat(self, repeat_times=2, interleave=True):
        ...
        if self.batch is not None:
            if interleave:
                ...
                repeated_tensors = {
                    key:
                        tensor.unsqueeze(0).expand(repeat_times,
                                                   *tensor.shape).reshape(-1, *tensor.shape[1:])
                    for key, tensor in self.batch.items()
                }
