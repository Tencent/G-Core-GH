...
if 'GymSokobanEnv' not in globals():
...
from gpatch_v4.agentic.env.sokoban.utils import generate_room
from gpatch_v4.agentic.env.utils import default_parser_action_func
from gpatch_v4.agentic.utils import all_seed
...
class SokobanEnv(Env, GymSokobanEnv):
    ...
    def render(self, mode=None):
        ...
        if render_mode == "text":
            ...
            return "\n".join(
                "".join(self.GRID_LOOKUP.get(cell, "?") for cell in row) for row in room.tolist()
            )
    ...
    def sample_random_action(self):
        return "<answer>Right</answer><|im_end|>"
...
if __name__ == "__main__":
    ...
    env: SokobanEnv = gem.make(
        env_id="sokoban", dim_room=(6, 6), num_boxes=1, max_steps=100, search_depth=10
    )
    for i in range(10):
