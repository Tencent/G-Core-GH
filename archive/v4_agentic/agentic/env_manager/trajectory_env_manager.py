...
by ``EnvAgentLoopActor`` via ``ThreadPoolExecutor``.
"""
...
class TrajEnvManager:
    ...
    def __init__(
        ...
        env_tool_config = rl_config.training.agentic.env_cfg_template.env_tool_config
    ...
    def _format_messages(self, history: RolloutCache) -> torch.Tensor:
        ...
        if (
            len(self.rollout_cache.history) > 1
            and self.rollout_cache.history[-2].get("use_tool", False)
        ):
    ...
    def _formulate_rollout_batch(
        ...
        all_messages = [item["messages"] for item in self.rollout_cache.history]
        log(f"[TrajEnvManager] _formulate_rollout_batch: {all_messages}", rank=0)

        scores = [item["reward"] for item in self.rollout_cache.history]
