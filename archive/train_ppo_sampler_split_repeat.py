...
# ```
@torch.no_grad()
async def gen_rollouts(engine, batch, sampling_repeat, sampling_repeat_idx):
    # batch 就是 actor 的 rollout_get_batch 的返回值，prompt_token_ids 是 list of dict，lpad_lens 和
    # gt_label 是 [b,] 的 tensor。
    args = get_args()
    assert args.generate_with_split_repeat, "it's a test for generate split repeat."
    ...
    for i in range(len(prompt_token_ids)):
        ...
        tmp_sampling_params.seed += i * sampling_repeat + sampling_repeat_idx
    ...
    for gi, gen in enumerate(gens):
        ...
        token = prompt_token_ids[gi]['prompt_token_ids'] + output_tokens
        ...
        prompt_lens.append(lpad_lens[gi])
        gt_label_list.append(gt_label[gi])
