"""Sokoban text env for validation (ported from roll_webshop)."""
...
from gpatch_v4.agentic.env.sokoban.sokoban_utils import generate_room
...
class SokobanEnv(Env, GymSokobanEnv):
    ...
    def reset(
        ...
        try:
            ...
            return self.reset(next_seed, data=data)
    ...
    def step(
        ...
        if not action_effective:
            action_desc = (
                f"At turn {self.num_env_steps}, you tried to move {action_info['action_content']}, "
                "which is not effective yet."
            )
        else:
            action_desc = (
                f"At turn {self.num_env_steps}, you moved {action_info['action_content']}, "
                "which is effective."
            )
