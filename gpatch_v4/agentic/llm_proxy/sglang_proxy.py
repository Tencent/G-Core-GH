...
class SglangProxy(BaseLLMProxy):
    ...
    def _http_generate_url(self) -> str:
        cfg = self._cfg()
        base = (cfg.get("sglang_base_url") or cfg.get("sglang_url") or
                _DEFAULT_HTTP_BASE).rstrip("/")
    ...
    def generate(self,
                 data: Dict[str, Any],
                 engine_index: Optional[int] = None) -> Optional[Dict[str, Any]]:
        del engine_index  # SglangProxy talks to a single endpoint; engine fan-out N/A.
        cfg = self._cfg()
