# adapted from https://github.com/verl-project/verl/blob/main/verl/utils/kernel/kernels.py
"""
...
from gpatch_v4.core.device import get_device_backend_name, is_cuda
...
if not HAVE_TRITON:
    ...
    def alloc_fn(size: int, alignment: int, stream: typing.Optional[int]):
        return torch.empty(size, device=get_device_backend_name(), dtype=torch.int8)
...
def get_torch_device():
    device_name = get_device_backend_name()
...
def efficient_entropy_triton_kernel_epilogue(
    ...
    for pid_n in range(0, tl.cdiv(num_splits, BLOCK_SIZE_N)):
        ...
        entropy_b_ptrs = entropy_b_ptr + offs_m[:, None] * stride_entropy_b_m + offs_n[
            None, :] * stride_entropy_b_n
        _entropy_b = tl.load(
            entropy_b_ptrs,
            mask=(offs_m[:, None] < num_tokens) & (offs_n[None, :] < num_splits),
            other=0.0
...
def efficient_entropy_triton_kernel_epilogue_tp(
    ...
    for pid_n in range(0, tl.cdiv(num_splits, BLOCK_SIZE_N)):
        ...
        _reduced_max = tl.load(
            reduced_max_ptr + offs_m[:, None] * stride_reduced_max_m +
            offs_n[None, :] * stride_reduced_max_n,
            mask=(offs_m[:, None] < num_tokens) & (offs_n[None, :] < num_splits),
            other=0.0,
        )
        _original_max = tl.load(
            original_max_ptr + offs_m[:, None] * stride_original_max_m +
        ...
        # update entropy_b
        _entropy_b = tl.load(
            entropy_b_ptr + offs_m[:, None] * stride_entropy_b_m +
...
def efficient_entropy_backward_kernel_general_mainloop_MN(
    ...
    d_hidden_ptrs = d_hidden_ptr + offs_am[:, None] * stride_d_hidden_m + offs_k[
        None, :] * stride_d_hidden_k
    # d_weight_ptrs = d_weight_ptr + offs_k[:, None] * stride_d_weight_k + offs_bn[None, :] * stride_d_weight_n
    d_weight_ptrs = d_weight_ptr + offs_bn[:, None] * stride_d_weight_n + offs_k[
        None, :] * stride_d_weight_k
    ...
    d_logits += d_entropy[:,
                          None] * (-exp_logits * accu_rcp[:, None]) * (logits - entropy_b[:, None])
...
def efficient_entropy_backward_kernel_d_hidden(
    ...
    for n in range(0, tl.cdiv(vocab_size, BLOCK_SIZE_N)):
        ...
        d_logits = d_logprobs[:, None] * (exp_logits * accu_rcp[:, None] - mask)
        d_logits += d_entropy[:, None] * (-exp_logits *
                                          accu_rcp[:, None]) * (logits - entropy_b[:, None])
    ...
    # write back
    tl.store(
        d_hidden_ptr + offs_m[:, None] * stride_d_hidden_m +
...
def efficient_entropy_backward_kernel_d_weight(
    ...
    # write back
    tl.store(
        d_weight_ptr + offs_n[:, None] * stride_d_weight_n +
...
def efficient_entropy_backward_kernel_general_d_logits(
    ...
    # store d_logits
    d_logits_ptrs = d_logits_ptr + offs_am[:, None] * stride_d_logits_m + offs_bn[
        None, :] * stride_d_logits_n
    tl.store(
        d_logits_ptrs,
...
def efficient_entropy_backward_kernel_general_d_logits_split_N(
    ...
    tl.store(
        d_logits_ptr + offs_am[:, None] * stride_d_logits_m +
        result_offs_n[None, :] * stride_d_logits_n, d_logits, mask
    )
...
def efficient_entropy_backward(
    ...
    if _config._backward == BackwardEnum._Total_Fuse_MN:
        ...
        _d_logits = torch.empty((num_tokens, vocab_size), device=hidden.device,
        ...
        for split_idx in range(num_splits):
            ...
            if split_idx == (num_splits - 1):
                vocab_right_bound = min(
                    (split_idx + 1) * vocab_per_split, vocab_size
