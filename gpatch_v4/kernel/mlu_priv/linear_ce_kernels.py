...
try:
    ...
    # TMA is NVIDIA Hopper-only; MLU uses Bang cores instead.
...
def get_entropy_reduction_enum(ce_reduction: int):
    """
...
def efficient_entropy_triton_kernel_epilogue(
    ...
    for i in tl.range(cnt):
        ...
        tl.store(
            ...
            tl.fdiv(global_entropy_b, global_accu),
            mask=mask_m,
        )
        tl.store(global_accu_ptr + offs_m_cur * stride_global_accu, global_accu, mask=mask_m)
        tl.store(
            global_entropy_ptr + offs_m_cur * stride_global_entropy,
            lse - tl.fdiv(global_entropy_b, global_accu),
            mask=mask_m,
        )
...
def epilogue_tp_update_config():
    ...
    for w, s in itertools.product(warps, stages):
        configs.append(triton.Config({"BLOCK_SIZE_M": 1024}, num_warps=w, num_stages=s))
...
def launch_efficient_entropy_backward_kernel_d_weight(
    ...
    if _d_weight_use_mouter(num_tokens, hidden_size):
        ...
        efficient_entropy_backward_kernel_d_weight_mouter[grid](
            ...
            dentropy.stride(0),
            dlogprobs,
            dlogprobs.stride(0) if dlogprobs.dim() > 0 else 0,
            reduction,
            entropy_b,
            entropy_b.stride(0),
            d_weight,
