...
from gpatch_v4.core.device import get_flash_attn_varlen_func
from gpatch_v4.models.bagel.modeling.bagel.cache_utils.taylorseer import (
    cal_type,
    derivative_approximation,
    taylor_cache_init,
    taylor_formula,
)
from gpatch_v4.models.bagel.modeling.bagel.qwen2 import Qwen2Config as _Qwen2Config
from gpatch_v4.models.bagel.modeling.bagel.qwen2.modeling_qwen2 import (
    Qwen2Attention,
    Qwen2MLP,
...
# triton is not supported by npu yet now
if is_cuda():
    flex_attention = torch.compile(flex_attention)
    Qwen2RMSNorm.forward = torch.compile(Qwen2RMSNorm.forward)
try:
...
class Qwen2Config(_Qwen2Config):
    ...
    def __init__(
        ...
        freeze_und=False,
        use_flash_mask=False,
        **kwargs,
    ):
        # transformers v5: Qwen2Config uses rope_parameters dict.
        if rope_parameters is not None:
            _rp = dict(rope_parameters)
            _rp.setdefault("rope_type", "default")
            _rp.setdefault("rope_theta", rope_theta)
        else:
            _rp = {"rope_type": "default", "rope_theta": rope_theta}
        ...
        self.use_flash_mask = use_flash_mask
...
def pad_sequence(tensor, pad_size, seq_dim=1):
    shape = list(tensor.shape)
    shape[seq_dim] = pad_size
    pad_tensor = tensor.new_zeros(*shape)
    return torch.cat([tensor, pad_tensor], dim=seq_dim)
...
class PackedAttention(Qwen2Attention):
    ...
    def forward_train(
        ...
        elif self.config.use_flash_mask:
            assert flashmask_attention is not None
            pad_size = sum(sample_lens) - packed_query_states.shape[0]
            packed_query_states = pad_sequence(packed_query_states, pad_size, seq_dim=0)
            packed_key_states = pad_sequence(packed_key_states, pad_size, seq_dim=0)
            packed_value_states = pad_sequence(packed_value_states, pad_size, seq_dim=0)
            packed_attn_output = flashmask_attention(
                packed_query_states.unsqueeze(0),  # 1, L, num_head, head_dim
                packed_key_states.unsqueeze(0),
                packed_value_states.unsqueeze(0),
                startend_row_indices=attention_mask,
            )
            end_index = packed_attn_output.shape[1] - pad_size
            packed_attn_output = packed_attn_output[0, :end_index, :, :].reshape(
                -1, self.hidden_size
            )
        else:
            pad_size = sum(sample_lens) - packed_query_states.shape[0]
    ...
    def forward_train(
        ...
        if self.config.freeze_und:
            packed_value_states[packed_und_token_indexes] = packed_value_states[
                packed_und_token_indexes].detach()
        ...
        if isinstance(attention_mask, List):
            ...
            assert flashmask_attention is not None
            pad_size = sum(sample_lens) - packed_query_states.shape[0]
            packed_query_states_ = pad_sequence(packed_query_states_, pad_size, seq_dim=0)
            packed_key_states_ = pad_sequence(packed_key_states_, pad_size, seq_dim=0)
            packed_value_states = pad_sequence(packed_value_states, pad_size, seq_dim=0)
            packed_attn_output = flashmask_attention(
                packed_query_states_.unsqueeze(0),  # 1, L, num_head, head_dim
...
class Qwen2MoTDecoderLayer(nn.Module):
    ...
    def forward_train(
        ...
        if self.freeze_und:
            packed_sequence_[packed_und_token_indexes] = packed_sequence_[packed_und_token_indexes
                                                                         ].detach()
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward_train(
        ...
        if self.config.freeze_und:
            packed_sequence[packed_und_token_indexes] = packed_sequence[packed_und_token_indexes
                                                                       ].detach()
