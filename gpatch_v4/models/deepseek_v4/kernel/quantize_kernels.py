...
def _fp4_qat_to_sgl_fp8_kernel(
    n: int,
    threads: int = 512,
):
...
def fp4_qat_to_sgl_fp8(weight: torch.Tensor, ) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if not weight.is_cuda:
        raise ValueError("fp4_qat_to_sgl_fp8 requires a CUDA tensor")
    ...
    kernel = _fp4_qat_to_sgl_fp8_kernel(n)
