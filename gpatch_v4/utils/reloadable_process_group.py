...
from gpatch_v4.utils import log, logging_memory_usage, logging_memory_usage_details
...
def monkey_patch_torch_dist():
    ...
    log("Applying monkey patch to torch.distributed")
    ...
    def new_group(*args, **kwargs):
        ...
        if len(args
              ) >= 3 and args[2] == "gloo" or "backend" in kwargs and kwargs["backend"] == "gloo":
            return group
    ...
    def get_new_function(func):
        def new_function(*args, **kwargs):
            args = tuple(
                [arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args]
            )
            kwargs = {
...
class ReloadableProcessGroup(torch.distributed.ProcessGroup):
    ...
    def destroy_process_groups():
        ...
        for reloadable_group in ReloadableProcessGroup.GROUPS.get(pid, []):
            ...
            try:
                ...
                log(f"Process group already invalid/destroyed; skipping cleanup. Exception: {e}")
    ...
    def reload_process_groups():
        ...
        log(f"Reloading {len(reloadable_groups)} process groups in pid {pid}")
    ...
    def broadcast_object_list(self, *a, **kw):
        return self._fwd("broadcast_object_list", *a, **kw)
...
def _wrap_low_level_call():
    try:
        ...
        mem_info = logging_memory_usage_details("after torch distributed error")
