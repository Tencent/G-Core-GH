...
class TimerSingleton:
    ...
    @classmethod
    def get_timer(cls):
        assert cls.timer is not None, "TimerSingleton is not initialized"
        return cls.timer

    @classmethod
    def create_timer(cls, report_config):
        cls._check_timer_not_inited()
        cls.timer = Timers(report_config.timing_log_level, report_config.timing_log_option)

    @classmethod
    def _check_timer_not_inited(cls):
        if cls.timer is not None:
            raise RuntimeError("TimerSingleton is already initialized")


def init_timer_singleton(report_config):
    logging_rank0("init_timer_singleton")
    TimerSingleton.create_timer(report_config)


def record_time_to_metrics(timers, time_log_keys, metrics, reset=True):
    for key in time_log_keys:
        metrics[f"time_perf/{key}"] = timers(key, log_level=0).elapsed(reset=reset)
    return metrics
