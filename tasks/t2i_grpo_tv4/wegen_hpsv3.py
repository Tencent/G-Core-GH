...
from gpatch_v4.models.wegen_hpsv3 import HPSv3RewardInferencer as WeGenHPSv3RewardInferencer


class WegenHpsv3RewardModel(BaseT2iRewardModel):
    # wegen HPSv3 的 code 跟环境依赖上有点冲突，目前能跑，而且与 sglang 不冲突的版本是:
    # ```
    ...
    def __init__(
        ...
        assert "hpsv3_yaml_path" in config.task, f"{config.task=}"

        self.model = WeGenHPSv3RewardInferencer(
            device=self.device,
            config_path=config.task["hpsv3_yaml_path"],
