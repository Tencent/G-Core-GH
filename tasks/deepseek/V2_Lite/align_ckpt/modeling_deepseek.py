...
class DeepseekV2YarnRotaryEmbedding(DeepseekV2RotaryEmbedding):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        inv_freq_mask = 1.0 - yarn_linear_ramp_mask(low, high, dim //
                                                    2).to(device=device, dtype=torch.float32)
            ...
            self.experts = nn.ModuleList(
                [
                    (
                        ...
                        i < (self.ep_rank + 1) * self.experts_per_rank else None
        ...
        if input_dtype == torch.float32:
            ...
            if hasattr(self.config, "_pre_quantization_dtype"):
                ...
                target_dtype = (
                    ...
                    if self.q_lora_rank is None else self.q_a_proj.weight.dtype
                )
...
class DeepseekV2Model(DeepseekV2PreTrainedModel):
    ...
    def forward(
        ...
        # print(f"DeepseekV2Model {inputs_embeds.shape=} {inputs_embeds=}")
...
class DeepseekV2ForCausalLM(DeepseekV2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # if torch.distributed.get_rank() == 0:
        # print(f"[DEBUG] {input_ids.shape=} {input_ids[0][:100]=} {inputs_embeds=}")
        ...
        # if torch.distributed.get_rank() == 0:
        # print(f"[DEBUG] {hidden_states.shape=} {hidden_states=} {logits.shape=} {logits=}")
        ...
        # print(f"[DEBUG] {loss=} {torch.sum(loss)}")
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            if (
                max_cache_length is not None and attention_mask is not None and
