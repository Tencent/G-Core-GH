...
# export PYTHONPATH="$PWD:../Megatron-LM-core-r0.14:$PYTHONPATH"
...
readonly SAVE_CHECKPOINT_DIR="$PWD/deepseek-v3-sft-fp8"
...
# readonly GLOBAL_BATCH_SIZE=256 # for align
readonly GLOBAL_BATCH_SIZE=4096 # for benchmark
...
"
    ...
    # --dataloader-type single \ # for align
DATA_ARGS="
    --tokenizer-type HuggingFaceTokenizer \
    --tokenizer-model ${TOKENIZER_MODEL} \
    --num-workers 1 \
    --dataloader-type cyclic \
    --data-path /mnt/ceph-sh2/user_jeffhong/workspace/wepsdl-dev/stanford_alpaca/dataset/alpaca_data.json \
    --use-map-dataset \
"
...
EVAL_AND_OUTPUT_ARGS="
    ...
    --wandb-exp-name $TS/deepseek-v3-mlm-align-seqlen-${SEQ_LENGTH}-PR-${PR}-tp-${TP_SIZE}-pp-${PP_SIZE}-cp-${CP_SIZE}-ep-${EP_SIZE}-lr-${LEARNING_RATE}-mbs-${MICRO_BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE} \
...
torchrun $DISTRIBUTED_ARGS $RUN_PY \
    ...
    --load $LOAD_CHECKPOINT_DIR >log/mlm-stanford-ds-v3-${NODE_RANK}.log 2>&1 &