...
def test_main(
    ...
    for return_recv_hook in (False, True):
        for dispatch_use_fp8 in (False, True):
            ...
            ) if dispatch_use_fp8 else packed_recv_x
            ...
            for i in range(num_local_experts if do_check else 0):
                ...
                recv_x = per_token_cast_back(packed_recv_x[0][i], packed_recv_x[1][i]
                                            ) if dispatch_use_fp8 else packed_recv_x[i]
                recv_count, recv_src_info, recv_layout_range = packed_recv_count[i], handle[0][
                    i], handle[1][i]
                ...
                for j in range(num_ranks):
                    begin_idx, count = (recv_layout_range[j] >>
                                        32).item(), (recv_layout_range[j] & int_mask).item()
                    assert (recv_x_amin == j -
                            rank_offset).sum().item() == (all_topk_idx[j] == expert_id).sum().item()
...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    for seed in range(int(1e9) if do_pressure_test else 0):
        ...
        ref_hash = test_main(
            num_tokens, hidden, num_experts, num_topk, rank, num_ranks, group, buffer, seed=seed
        )
        for i in range(20):
            assert test_main(
                num_tokens,
