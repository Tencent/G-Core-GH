...
def test_main(
    num_sms: int, local_rank: int, num_local_ranks: int, num_ranks: int, num_nodes: int, rank: int,
    buffer: deep_ep.Buffer, group: dist.ProcessGroup
):
    # Settings
    num_tokens, hidden, num_topk_groups, num_topk, num_experts = 4096, 7168, min(
        num_nodes, 4
    ), 8, (256 // num_ranks) * num_ranks
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_e4m3):
                for with_topk in (False, True):
                        ...
                        dispatch_args.update(
                            {
                                ...
                                'topk_weights':
                                    ...
                                    if current_x is x_pure_rand else topk_weights
                            }
                        )
                    if previous_mode:
                    ...
                    assert gbl_num_tokens_per_expert.view(
                        num_ranks, -1
                    ...
                    if not with_topk:
                        ...
                        event.current_stream_wait() if async_mode else ()
                        recv_x = per_token_cast_back(*recv_x
                                                    ) if isinstance(recv_x, tuple) else recv_x
                        if current_x is not x_pure_rand:
                            check_data(recv_x, recv_gbl_rank_prefix_sum)
                    ...
                    if with_topk:
                        check_topk_weights = combined_topk_weights if (
                            current_x is x_pure_rand
    ...
    for current_x in (x_e4m3, x):
        ...
        rdma_send_bytes = (dispatch_bf16_rdma_send_bytes * fp8_factor
                          ) if isinstance(current_x, tuple) else dispatch_bf16_rdma_send_bytes
        nvl_recv_bytes = (dispatch_bf16_nvl_recv_bytes * fp8_factor
                         ) if isinstance(current_x, tuple) else dispatch_bf16_nvl_recv_bytes
        for nvl_chunk_size in range(4, 33, 4):
            for rdma_chunk_size in range(4, 33, 4):
                # config = deep_ep.Config(num_sms, nvl_chunk_size, nvl_buffer_size, rdma_chunk_size, rdma_buffer_size)
                tune_args = {'x': current_x, 'handle': handle, 'config': None}
    ...
    dispatch_args = {
        ...
        'num_tokens_per_expert': num_tokens_per_expert,
        'config': None
    }  # dispatch_config if dispatch_config is not None else config
    ...
    for nvl_chunk_size in range(1, 5, 1):
        for rdma_chunk_size in range(8, 33, 4):
            # config = deep_ep.Config(num_sms, nvl_chunk_size, nvl_buffer_size, rdma_chunk_size, rdma_buffer_size)
            tune_args = {'x': recv_x, 'handle': handle, 'config': None}
