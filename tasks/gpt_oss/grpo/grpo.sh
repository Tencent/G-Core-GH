...
readonly GRPO_RUN="test_run"

readonly DFS_PATH="/mnt/ceph-hz1-csp/mm-base-plt2"
# readonly DFS_PATH="/mnt/gemininjceph2/geminicephfs/mm-base-plt2"
readonly ACTOR_TOKENIZER_MODEL="$DFS_PATH/nrwu/hf-hub/lmsys/gpt-oss-20b-bf16"
readonly RM_TOKENIZER_MODELS="$DFS_PATH/nrwu/hf-hub/lmsys/gpt-oss-20b-bf16"
readonly DATA_PATH="tasks/gpt_oss/rl_metadata.json"

# 对每个角色设置加载路径
if [ "$PPO_ROLE" = "actor" ]; then
  readonly MODEL_YAML="gpatch/model_yamls/gpt-oss-20b.yaml"
  readonly REF_LOAD_CHECKPOINT_DIR="/mnt/ceph-hz1-csp/mm-base-plt2/user_astrachang/code/gcore-dev/gpt-oss-20b-bf16-mlm"
  readonly LOAD_CHECKPOINT_DIR="${PWD}/gpt-oss-20b-bf16-mlm"
  readonly SAVE_CHECKPOINT_DIR="${PWD}/gpt-oss-20b-save4"
  ...
  readonly MODEL_YAML="gpatch/model_yamls/gpt-oss-20b.yaml"
  readonly LOAD_CHECKPOINT_DIR="${PWD}/gpt-oss-mlm"
  ...
  readonly MODEL_YAML="gpatch/model_yamls/gpt-oss-20b.yaml"
  readonly LOAD_CHECKPOINT_DIR="${DFS_PATH}/nrwu/hf-hub/lmsys/gpt-oss-20b-bf16"
  readonly REF_LOAD_CHECKPOINT_DIR=$LOAD_CHECKPOINT_DIR
  readonly SAVE_CHECKPOINT_DIR="none"
  readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
...
readonly SEQ_LENGTH=$((8*1024))
readonly RESP_SEQ_LENGTH=$((1*1024))
...
CRITIC_SVR_IPS $CRITIC_SVR_IPS
CRITIC_SVR_PORTS $CRITIC_SVR_PORTS
"
...
if [ "$PPO_ROLE" = "sampler" ] || [ "$PPO_ROLE" = "gen-rm" ] || [ "$PPO_ROLE" = "critic" ]; then
  RL_ARGS="$RL_ARGS
      --no-fused-kernel \
  "
fi
...
DEBUG_ARGS=""
# DEBUG_ARGS="
#   --ppo-debug-sglang-sleep-wakeup-generate \
# "
# DEBUG_ARGS="
#   --ppo-debug-update-weight \
# "
...
"
if [ "$PPO_ROLE" = "actor" ]; then
  RUN_PY='./tasks/gpt_oss/grpo/train_ppo_actor.py'
elif [ "$PPO_ROLE" = "critic" ]; then
  RUN_PY='./tasks/gpt_oss/grpo/train_ppo_critic.py'
elif [ "$PPO_ROLE" = "sampler" ]; then
  RUN_PY='./tasks/gpt_oss/grpo/train_ppo_sampler.py'
else
  echo "no ${PPO_ROLE}"
  exit 0
fi
