...
if [[ $# -ne 1 ]]; then
    echo "Usage: bash tasks/math_rl_v4/scripts/grpo_entropy_ctrl.sh {clip-cov|kl-cov}" >&2
    exit 2
fi

readonly ENTROPY_TYPE="$1"
case "$ENTROPY_TYPE" in
    clip-cov)
        readonly EXP_NAME="new_loss_global_clip_cov"
        ENTROPY_ARGS=(
            +ppo.ppo_clip_cov_ratio=0.0002
        )
        ;;
    kl-cov)
        readonly EXP_NAME="new_loss_global_kl_cov"
        ENTROPY_ARGS=(
...
# bash tasks/math_rl_v4/scripts/grpo_entropy_ctrl.sh clip-cov > logs/new_loss_global_clip_cov.log 2>&1 &
# bash tasks/math_rl_v4/scripts/grpo_entropy_ctrl.sh kl-cov > logs/new_loss_global_kl_cov.log 2>&1 &
python3 -u gpatch_v4/entry/train_lm_grpo.py \
    ...
    +ppo.use_legacy_loss=false \
    +ppo.ppo_entropy_global_cov=true \
    +ppo.ppo_entropy_regularization_type="${ENTROPY_TYPE}" \
    "${ENTROPY_ARGS[@]}"
