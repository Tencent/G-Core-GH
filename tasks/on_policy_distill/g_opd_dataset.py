...
class G_OpdDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        #TODO: 注意，这里只是为了模拟，实际上 teacher_type 的字段应该定义在数据集每一条样子里
        teacher_type = "big_teacher" if random.random() < 0.7 else "small_teacher"
...
def collate_fn(examples):
    ...
    for example in examples:
        ...
        teacher_type_list.append(teacher_type)

    return {
        "prompt_token_ids": prompt_token_ids,
        "prompt_lens": prompt_lens,
        "gt_label": gt_label,
        "teacher_type": teacher_type_list,
        "cache_keys": ["teacher_type"],
    }


def get_dataset_and_dataloader(config: RlConfig = None, tokenizer=None, dp_rank=0, dp_size=1):
    dataset = G_OpdDataset(config=config, tokenizer=tokenizer)
