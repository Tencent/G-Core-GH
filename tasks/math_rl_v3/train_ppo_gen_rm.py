...
async def gen_rm_func(engine, batch: Optional[Dict[str, Union[int, List[Any]]]]):
    ...
    # 现在有三层关系 hetero_genrm -> rm_idx -> prompt_idx
    # 对于每个 hetero gen-rm 可能存在多个不同的 rm ckpts （rm_idx）对于同一个 rm ckpts 存在不同的 prompt （prompt_idx）
    # 这些都是 global 的编号，所以需要自己算 offset，这个case下每个 rm_idx 唯一对应一个 prompt_idx，所以这里是对的
    rm_idx = sum(
        args.ppo_num_rm_per_hetero_gen_rm[:args.ppo_hetero_gen_rm_idx]
    ) + engine.model_index
    assert f'rm_{rm_idx}_tokens' in batch
    prompt_token_ids = batch[f'rm_{rm_idx}_tokens']
    ...
    for i in range(len(prompt_token_ids)):
        for j in range(args.ppo_gen_rm_repeat):
            ...
            gen = engine.async_generate(
                {'prompt_token_ids': prompt_token_ids[i].tolist()}, tmp_sampling_params,
                str(uuid.uuid4().hex)
            )
            gens.append(gen)
