...
export PYTHONPATH="$PWD:$PWD/../Megatron-LM:../mbridge:$PYTHONPATH"
...
source $PWD/tasks/deepseek/V3/deepep_env.sh
...
ACTOR_ETP_SIZE=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn actor-etp-size`
...
readonly USERNAME="jeffhong"
readonly TS=$(date +%Y-%m-%d-%H-%M-%S)
readonly GRPO_RUN="deepseek-v3-grpo-${TS}"

readonly DFS_PATH="/mnt/ceph-sh2"
# readonly DFS_PATH="/mnt/gemininjceph2/geminicephfs/mm-base-plt2"
readonly ACTOR_TOKENIZER_MODEL="${DFS_PATH}/user_jeffhong/hf-hub/deepseek-ai/DeepSeek-V3"
readonly RM_TOKENIZER_MODELS="${DFS_PATH}/user_jeffhong/hf-hub/deepseek-ai/DeepSeek-V3"
readonly data_path="tasks/math_rl_v3/deepseek-v3/rl_metadata_sh.json"
# readonly eval_data_path="tasks/deepseek/math_rl_v3/rl_eval_metadata_sh.json"

# 对每个角色设置加载路径
if [ "$PPO_ROLE" = "actor" ]; then
  readonly MODEL_YAML="gpatch/model_yamls/deepseek-v3.yaml"
  # readonly LOAD_CHECKPOINT_DIR="${PWD}/xxx"
  # readonly REF_LOAD_CHECKPOINT_DIR="${PWD}/xxx"
  readonly LOAD_CHECKPOINT_DIR="${PWD}/deepseek-v3-mlm-ckpt"
  readonly REF_LOAD_CHECKPOINT_DIR="${PWD}/deepseek-v3-mlm-ckpt"
  readonly SAVE_CHECKPOINT_DIR="${PWD}/deepseek-v3-grpo-save-ckpt"
  ...
  readonly MODEL_YAML="gpatch/model_yamls/deepseek-v3.yaml"
#   readonly LOAD_CHECKPOINT_DIR="${DFS_PATH}/user_jeffhong/hf-hub/deepseek-ai/DeepSeek-V3"
  readonly LOAD_CHECKPOINT_DIR="${DFS_PATH}/user_jeffhong/hf-hub/deepseek-ai/DeepSeek-V3-bf16"
  readonly REF_LOAD_CHECKPOINT_DIR=${LOAD_CHECKPOINT_DIR}
  readonly SAVE_CHECKPOINT_DIR="none"
  readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
...
readonly RESP_SEQ_LENGTH=$((2*1024))
...
if [ "$PPO_ROLE" = "actor" ]; then
    MP_ARGS="${MP_ARGS} --expert-tensor-parallel-size $ETP_SIZE "
    MP_ARGS="${MP_ARGS} \
    ...
    if [[ ${PP_SIZE} -gt 1 ]]; then
        ...
        elif [[ ${PP_SIZE} -eq 12 ]]; then
            MP_ARGS="${MP_ARGS} --decoder-first-pipeline-num-layers 6 "
...
RL_ARGS="$GEN_ARGS
    ...
    --ppo-sampler-client-update-timeout 180 \
"
