...
readonly ACTOR_TOKENIZER_MODEL="$HF_HUB_DIR"
...
# 对每个角色设置加载路径
if [ "$PPO_ROLE" = "actor" ]; then
  readonly MODEL_YAML="gpatch/model_yamls/qwen3-coder-next-80b.yaml"
  readonly REF_LOAD_CHECKPOINT_DIR="$HF_HUB_DIR"
  readonly LOAD_CHECKPOINT_DIR="$HF_HUB_DIR"
  readonly SAVE_CHECKPOINT_DIR="${PWD}/qwen3_next_coder_80b_a3b_grpo_save"
...
if [ "$PPO_ROLE" = "actor" ]; then
    OUTPUT_ARGS="$OUTPUT_ARGS
      ...
      --wandb-exp-name qwen-coder-next-grpo-tp$TP_SIZE-pp$PP_SIZE-cp$CP_SIZE-ep$EP_SIZE \
...
if [ "$PPO_ROLE" = "actor" ]; then
  RUN_PY='./tasks/math_rl_v3/qwen3/moe/coder_next/train_qwen3_coder_next_ppo_actor.py'
elif [ "$PPO_ROLE" = "critic" ]; then
