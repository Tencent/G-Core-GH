...
readonly gdpo_reward_weights='{"acc_rewards":1.0,"fmt_rewards":1.0}'
...
OUTPUT_ARGS="
    ...
    --no-load-rng \
    --wandb-project qwen3 \
    --wandb-exp-name icepop_gdpo \
    --wandb-save-dir  wandb_local \
"
# 上报参数
...
RL_ARGS="
    ...
    --ppo-use-absolute-kl \
    --use-grpo \
    --gdpo-reward-weights "$gdpo_reward_weights" \
    --enable-gdpo \
    --grpo-advantage-epsilon 1e-8 \
    ...
    --off-policy-correction-mode mask \
    --off-policy-correction-level token \
    --off-policy-correction-lower-bound 0.5 \
    --off-policy-correction-upper-bound 5.0 \
"
