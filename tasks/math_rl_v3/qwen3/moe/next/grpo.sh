...
export PYTHONPATH="$PWD:$PWD/../3rdparty/Megatron-LM/:$PYTHONPATH"
...
readonly ACTOR_TOKENIZER_MODEL="$DFS_PATH/nrwu/hf-hub/Qwen/Qwen3-Next-80B-A3B-Instruct"
readonly RM_TOKENIZER_MODELS="${DFS_PATH}/nrwu/hf-hub/Qwen/Qwen2.5-Math-RM-72B"
readonly DATA_PATH="tasks/math_rl_v3/qwen/rl_metadata.json"

# 对每个角色设置加载路径
if [ "$PPO_ROLE" = "actor" ]; then
  readonly MODEL_YAML="gpatch/model_yamls/qwen3-next-80b-a3b.yaml"
  readonly REF_LOAD_CHECKPOINT_DIR="${PWD}/qwen-next-mlm"
  readonly LOAD_CHECKPOINT_DIR="${PWD}/qwen-next-mlm"
  ...
  readonly MODEL_YAML="gpatch/model_yamls/qwen3-next-80b-a3b.yaml"
  readonly LOAD_CHECKPOINT_DIR="$DFS_PATH/nrwu/hf-hub/Qwen/Qwen3-Next-80B-A3B-Instruct"
  readonly REF_LOAD_CHECKPOINT_DIR=$LOAD_CHECKPOINT_DIR
  readonly SAVE_CHECKPOINT_DIR="none"
  readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
...
RL_ARGS="$GEN_ARGS
    ...
    --ppo-sampling-repeat 16 \
    --ppo-sampling-keep 16 \
    --ppo-use-absolute-kl \
    --use-grpo \
