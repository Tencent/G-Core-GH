...
MLM_PATH="/root/Megatron-LM"
MBRIDGE_PATH="/root/mbridge"
MCORE_BRIDGE_PATH="/root/Megatron-Bridge/src"
export PYTHONPATH="$PWD:$MLM_PATH:$MBRIDGE_PATH:$MCORE_BRIDGE_PATH:$PYTHONPATH"
...
readonly HF_HUB_DIR="${PWD}/hf-hub/Qwen/Qwen3.5-2B/"
...
if [ "$PPO_ROLE" = "actor" ]; then
    readonly MODEL_YAML="gpatch/model_yamls/qwen3.5-2b.yaml"
    readonly LOAD_CHECKPOINT_DIR=$HF_HUB_DIR
    readonly REF_LOAD_CHECKPOINT_DIR=$HF_HUB_DIR
    readonly SAVE_CHECKPOINT_DIR="${PWD}/qwen_3_5_2b_save"
...
readonly ROLLOUT_GLOBAL_BATCH_SIZE=32
readonly PARTIAL_ROLLOUT_GBS=32
...
if [ "$PPO_ROLE" = "actor" ]; then
    RUN_PY='./tasks/math_rl_v3/train_ppo_actor_qwen3_5.py'
elif [ "$PPO_ROLE" = "critic" ]; then
