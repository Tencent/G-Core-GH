PLACE_CFG_FOLDER=$PWD/place-config-qwen2p5-fp8
LOG_DIR="$PWD/log/math_rl_fp8"
PART=1
...
SHELL=tasks/math_rl_v3/fp8_align/qwen/grpo_fp8.sh
...
mpirun -v --allow-run-as-root \
      ...
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash $SHELL $PLACE_CFG_FOLDER sampler >$LOG_DIR/sampler$PART.log 2>&1 &
SAMPLER_PID=$!
...
mpirun -v --allow-run-as-root \
      ...
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash $SHELL $PLACE_CFG_FOLDER critic >$LOG_DIR/critic$PART.log 2>&1 &
RM_PID=$!
...
mpirun -v --allow-run-as-root \
      ...
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash $SHELL $PLACE_CFG_FOLDER actor >$LOG_DIR/actor$PART.log 2>&1 &
ACTOR_PID=$!
