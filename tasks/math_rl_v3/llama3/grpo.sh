...
readonly WANDB_PROJECT="test_run"
readonly WANDB_EXP_NAME="llama3-grpo"
readonly DFS_PATH="/mnt/ceph-hz1-csp/mm-base-plt2"
readonly ACTOR_TOKENIZER_MODEL="${DFS_PATH}/nrwu/hf-hub/unsloth/Llama-3.2-1B/"
readonly RM_TOKENIZER_MODELS="${DFS_PATH}/nrwu/hf-hub/unsloth/Llama-3.2-1B/"
readonly data_path="tasks/math_rl_v3/llama3/rl_metadata_hz.json"
readonly eval_data_path="tasks/math_rl_v3/llama3/rl_eval_metadata_hz.json"

if [ "$PPO_ROLE" = "actor" ]; then
    readonly MODEL_YAML="gpatch/model_yamls/llama3.2-1b.yaml"
    readonly LOAD_CHECKPOINT_DIR="${PWD}/llama_3_2_1b"
    readonly REF_LOAD_CHECKPOINT_DIR="${PWD}/llama_3_2_1b_ref"
    readonly SAVE_CHECKPOINT_DIR="${PWD}/llama_3_2_1b_save"
    readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
    readonly TB_DIR="tb/grpo-actor"
    readonly WANDB_DIR="wandb_local/grpo-actor"
    readonly LR=5e-7
elif [ "$PPO_ROLE" = "critic" ]; then
    readonly MODEL_YAML="gpatch/model_yamls/llama3.2-1b.yaml"   # 业务需求为 rule-only，但不能为 "none"，随便填的
    ...
    readonly LOAD_CHECKPOINT_DIR="${DFS_PATH}/nrwu/hf-hub/unsloth/Llama-3.2-1B/"
    readonly REF_LOAD_CHECKPOINT_DIR=${LOAD_CHECKPOINT_DIR}
    readonly SAVE_CHECKPOINT_DIR="none"
    readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
...
readonly PPO_STEP_EVAL_INTERVAL=4
...
RL_ARGS="$GEN_ARGS
    ...
    --ppo-smart-pad-train \
"
    # --ppo-train-dynamic-mbs-target-seq 2048 \
