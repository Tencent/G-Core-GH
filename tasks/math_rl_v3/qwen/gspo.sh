#!/bin/bash
# align with https://github.com/volcengine/verl/blob/main/recipe/gspo/test_gspo_3b_math.sh(not yet)
...
readonly WANDB_PROJECT="yeazhao_gspo"
readonly WANDB_EXP_NAME="qwen2.5-gspo-gdatasetv4-vllm-rm_with_rule"
...
if [ "$PPO_ROLE" = "actor" ]; then
    ...
    readonly LOAD_CHECKPOINT_DIR="${PWD}/qwen_2_5_1_5b_sft"
    ...
    readonly TB_DIR="tb/gspo-actor"
    readonly WANDB_DIR="wandb_local/gspo-actor"
    readonly LR=5e-7
elif [ "$PPO_ROLE" = "critic" ]; then
    ...
    readonly TB_DIR="tb/gspo-critic"
    readonly WANDB_DIR="wandb_local/gspo-critic"
    readonly LR=5e-7
elif [ "$PPO_ROLE" = "sampler" ]; then
    readonly MODEL_YAML="gpatch/model_yamls/qwen2.5-math-1.5b.yaml"
    ...
    readonly WANDB_DIR="wandb_local/gspo-sampler"
    readonly LR=5e-7
else
    echo "$PPO_ROLE no support"
    exit 0
