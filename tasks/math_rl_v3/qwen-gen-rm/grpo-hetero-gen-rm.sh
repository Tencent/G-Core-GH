...
# GEN_RM_IDX 参数检查：gen-rm 角色必须指定该参数
if [ "$PPO_ROLE" = "gen-rm" ]; then
    if [ -z "$3" ]; then
        echo "ERROR: GEN_RM_IDX (3rd argument) is required for gen-rm role"
        echo "Usage: $0 <PLACE_CFG_FOLDER> gen-rm <GEN_RM_IDX>"
        exit 1
    fi
fi
readonly GEN_RM_IDX=${3:-0}  # 默认使用第0个 gen-rm
...
HETERO_GEN_RM_NUM=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn hetero-gen-rm-num`

# 声明数组存储每个异构 gen-rm 的配置
declare -a GEN_RM_NNODES_ARR
declare -a GEN_RM_MASTER_ADDR_ARR
declare -a GEN_RM_MASTER_PORT_ARR
declare -a GEN_RM_SVR_IPS_ARR
declare -a GEN_RM_SVR_PORTS_ARR
declare -a GEN_RM_DIST_INIT_ADDRS_ARR
declare -a GEN_RM_TP_SIZE_ARR
declare -a GEN_RM_PP_SIZE_ARR
declare -a GEN_RM_DP_SIZE_ARR
declare -a GEN_RM_EP_SIZE_ARR

for ((i=0; i<$HETERO_GEN_RM_NUM; i++)); do
    GEN_RM_NNODES_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-nnodes --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_MASTER_ADDR_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-master-addr --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_MASTER_PORT_ARR[$i]=$((65500 + i))
    GEN_RM_SVR_IPS_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-svr-ips --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_SVR_PORTS_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-svr-ports --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_DIST_INIT_ADDRS_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-dist-init-addrs --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_TP_SIZE_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-tp-size --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_PP_SIZE_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-pp-size --get-fn-hetero-gen-rm-idx $i`
    GEN_RM_EP_SIZE_ARR[$i]=`python tools/auto_place.py --fn get --config-folder $PLACE_CFG_FOLDER --get-fn gen-rm-ep-size --get-fn-hetero-gen-rm-idx $i`
    # 计算每个 gen-rm 的 DP size
    GEN_RM_DP_SIZE_ARR[$i]=$(($GPUS_PER_NODE*${GEN_RM_NNODES_ARR[$i]}/${GEN_RM_TP_SIZE_ARR[$i]}/${GEN_RM_PP_SIZE_ARR[$i]}))
    if [ "$PPO_ROLE" = "gen-rm" ] && [ $GEN_RM_IDX -eq $i ]; then
        echo "Hetero GEN-RM $i config:"
        echo "  NNODES: ${GEN_RM_NNODES_ARR[$i]}"
        echo "  MASTER_ADDR: ${GEN_RM_MASTER_ADDR_ARR[$i]}"
        echo "  MASTER_PORT: ${GEN_RM_MASTER_PORT_ARR[$i]}"
        echo "  SVR_IPS: ${GEN_RM_SVR_IPS_ARR[$i]}"
        echo "  SVR_PORTS: ${GEN_RM_SVR_PORTS_ARR[$i]}"
        echo "  TP_SIZE: ${GEN_RM_TP_SIZE_ARR[$i]}"
        echo "  PP_SIZE: ${GEN_RM_PP_SIZE_ARR[$i]}"
        echo "  EP_SIZE: ${GEN_RM_EP_SIZE_ARR[$i]}"
        echo "  DP_SIZE: ${GEN_RM_DP_SIZE_ARR[$i]}"
    fi
done

# 验证 GEN_RM_IDX 参数
if [ "$PPO_ROLE" = "gen-rm" ]; then
    if [ $GEN_RM_IDX -ge $HETERO_GEN_RM_NUM ]; then
        echo "ERROR: GEN_RM_IDX ($GEN_RM_IDX) >= HETERO_GEN_RM_NUM ($HETERO_GEN_RM_NUM)"
        exit 1
    fi
    if [ $GEN_RM_IDX -lt 0 ]; then
        echo "ERROR: GEN_RM_IDX ($GEN_RM_IDX) < 0"
        exit 1
    fi
    echo "Using GEN-RM instance: $GEN_RM_IDX"
fi

# Concatenate all gen-rm configs (space separated)
ALL_GEN_RM_SVR_IPS="${GEN_RM_SVR_IPS_ARR[*]}"
ALL_GEN_RM_SVR_PORTS="${GEN_RM_SVR_PORTS_ARR[*]}"
ALL_GEN_RM_PP_SIZE="${GEN_RM_PP_SIZE_ARR[*]}"
ALL_GEN_RM_TP_SIZE="${GEN_RM_TP_SIZE_ARR[*]}"
ALL_GEN_RM_DP_SIZE="${GEN_RM_DP_SIZE_ARR[*]}"
ALL_GEN_RM_DIST_INIT_ADDRS="${GEN_RM_DIST_INIT_ADDRS_ARR[*]}"

echo "[DEBUG] ALL_GEN_RM_SVR_IPS: $ALL_GEN_RM_SVR_IPS"



# 根据 GEN_RM_IDX 选择对应的 gen-rm 配置
GEN_RM_NNODES=${GEN_RM_NNODES_ARR[$GEN_RM_IDX]}
GEN_RM_MASTER_ADDR=${GEN_RM_MASTER_ADDR_ARR[$GEN_RM_IDX]}
GEN_RM_MASTER_PORT=${GEN_RM_MASTER_PORT_ARR[$GEN_RM_IDX]}
GEN_RM_SVR_IPS=${GEN_RM_SVR_IPS_ARR[$GEN_RM_IDX]}
GEN_RM_SVR_PORTS=${GEN_RM_SVR_PORTS_ARR[$GEN_RM_IDX]}
GEN_RM_DIST_INIT_ADDRS=${GEN_RM_DIST_INIT_ADDRS_ARR[$GEN_RM_IDX]}
GEN_RM_TP_SIZE=${GEN_RM_TP_SIZE_ARR[$GEN_RM_IDX]}
GEN_RM_PP_SIZE=${GEN_RM_PP_SIZE_ARR[$GEN_RM_IDX]}
GEN_RM_DP_SIZE=${GEN_RM_DP_SIZE_ARR[$GEN_RM_IDX]}
GEN_RM_EP_SIZE=${GEN_RM_EP_SIZE_ARR[$GEN_RM_IDX]}

echo "Selected GEN-RM $GEN_RM_IDX config:"
echo "  NNODES: $GEN_RM_NNODES"
echo "  MASTER_ADDR: $GEN_RM_MASTER_ADDR"
echo "  MASTER_PORT: $GEN_RM_MASTER_PORT"
echo "  SVR_IPS: $GEN_RM_SVR_IPS"
echo "  SVR_PORTS: $GEN_RM_SVR_PORTS"
echo "  TP_SIZE: $GEN_RM_TP_SIZE"
echo "  PP_SIZE: $GEN_RM_PP_SIZE"
echo "  EP_SIZE: $GEN_RM_EP_SIZE"
echo "  DP_SIZE: $GEN_RM_DP_SIZE"
...
if [ "$PPO_ROLE" = "actor" ]; then
    ...
    readonly MASTER_PORT=${GEN_RM_MASTER_PORT:-65535}
...
readonly RM_TOKENIZER_MODELS="${PWD}/hf-hub/Qwen/Qwen2.5-Math-72B-Instruct/ ${PWD}/hf-hub/Qwen/Qwen3-Next-80B-A3B-Instruct/"
...
rm_load+=("${PWD}/hf-hub/Qwen/Qwen2.5-Math-72B-Instruct")
rm_load+=("${PWD}/hf-hub/Qwen/Qwen3-Next-80B-A3B-Instruct")
rm_load="${rm_load[*]}"
...
if [ "$PPO_ROLE" = "actor" ]; then
    ...
    readonly REF_LOAD_CHECKPOINT_DIR="${PWD}/qwen_2_5_1_5b"
    ...
    readonly REF_LOAD_CHECKPOINT_DIR=${rm_load}
    readonly SAVE_CHECKPOINT_DIR="none"
    readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
    readonly TB_DIR="tb/grpo-gen-rm-${GEN_RM_IDX}"
    readonly WANDB_DIR="wandb_local/grpo-gen-rm-${GEN_RM_IDX}"
    readonly LR=5e-7

    # 悄悄干点事情
    ALL_GEN_RM_SVR_IPS=${GEN_RM_SVR_IPS}
    ALL_GEN_RM_SVR_PORTS=${GEN_RM_SVR_PORTS} 
elif [ "$PPO_ROLE" = "sampler" ]; then
    readonly MODEL_YAML="gpatch/model_yamls/qwen2.5-math-1.5b.yaml"
...
# 确定 topo
# 注意：GEN_RM_DP_SIZE 已经在上面根据 GEN_RM_IDX 选择好了
...
if [ "$PPO_ROLE" = "actor" ]; then
    ...
    readonly EP_SIZE=$GEN_RM_EP_SIZE
    readonly CP_SIZE=1
...
readonly ROLLOUT_GLOBAL_BATCH_SIZE=16
...
RL_ARGS="$GEN_ARGS
    ...
    --ppo-gen-rm-ips $ALL_GEN_RM_SVR_IPS \
    --ppo-gen-rm-ports $ALL_GEN_RM_SVR_PORTS \
    --ppo-gen-rm-pipeline-model-parallel-size $GEN_RM_PP_SIZE \
    --ppo-gen-rm-tensor-model-parallel-size $GEN_RM_TP_SIZE \
    --ppo-gen-rm-expert-model-parallel-size $GEN_RM_EP_SIZE \
    --ppo-gen-rm-data-parallel-size $ALL_GEN_RM_DP_SIZE \
    ...
    --ppo-gen-rm-hetero-num $HETERO_GEN_RM_NUM \
    --ppo-num-rm 2 \
    --ppo-num-rm-per-hetero-gen-rm 1 1 \
    --ppo-hetero-gen-rm-idx $GEN_RM_IDX \
    --ppo-destroy-gen-rm-when-offload \
"
...
if [ "$PPO_ROLE" = "gen-rm" ]; then
  # 如果大量用235B模型做gen-rm， --rsync-ckpts-to-shm 可能会引起 OOM，最多3个235B模型
  RL_ARGS="$RL_ARGS
      --rsync-ckpts-to-shm \
  "
fi
