...
readonly DATASET_META="/tmp/pmc_vqa_grpo_dataset_v4.json"
...
readonly SEQ_LENGTH=6144
# readonly RESP_SEQ_LENGTH=2048
readonly RESP_SEQ_LENGTH=3072
...
GEN_ARGS="
    ...
    --max-running-requests 220 \
    --sampler-gpu-memory-utilization 0.55 \
"
...
if [ "$PPO_ROLE" = "actor" ]; then
  RUN_PY='./tasks/router_replay/train_qwen3vl_ppo_actor.py'
elif [ "$PPO_ROLE" = "critic" ]; then
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  EXTRA_ARGS+=" --wandb-exp-name grpo/rule-only/4b-dense/pmc-vqa/Instruct/baseline "
  EXTRA_ARGS+=" --wandb-save-dir $WANDB_DIR "
elif [ "$PPO_ROLE" = "critic" ]; then
  EXTRA_ARGS+=" --ppo-mm-rule-type image_pair "
fi
