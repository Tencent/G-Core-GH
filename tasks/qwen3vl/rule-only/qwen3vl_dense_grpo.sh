...
MYWD=$PWD
readonly HF_HUB_DIR="$MYWD/hf-hub/Qwen/Qwen3-VL-2B-Instruct"
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  readonly SAVE_CHECKPOINT_DIR="${PWD}/ckpt_qwen3vl_4b_grpo_actor_save"
...
readonly DATASET_META="/tmp/geometry3k_grpo_dataset_v3.json"
if [ "$PPO_ROLE" = "actor" ]; then
    python tools/data_convert/build_dataset_v3_meta.py \
        --output_fullpath $DATASET_META \
        --dataset_dir $DATASET_ROOT/train \
        --dataset_eval_dir $DATASET_ROOT/test \
        --rebuild
fi
DATASET_ARGS="
...
if [ "$PPO_ROLE" = "critic" ]; then
  readonly MODEL_YAML="gpatch/model_yamls/qwen3vl-2b.yaml"
...
if [ "$PPO_ROLE" = "actor" ]; then
  readonly GLOBAL_BATCH_SIZE=640
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  RUN_PY='./tasks/qwen3vl/train_qwen3vl_ppo_critic.py'
elif [ "$PPO_ROLE" = "sampler" ]; then
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  EXTRA_ARGS+=" --wandb-exp-name grpo/rule-only/4b-dense/geometry3k/Instruct/baseline "
...
EVAL_ARGS="
    --ppo-step-eval-interval 5 \
    --ppo-eval-steps 1 \
    --ppo-eval-rollout-global-batch-size 576 \
