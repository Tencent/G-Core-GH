...
LOG_DIR="$PWD/log/qwen3vl-geometry3k-v2"
PART=1
...
mpirun -v --allow-run-as-root \
      ...
      --sampler-tp-size 1 --sampler-pp-size 1 \
      --critic-tp-size 1 --critic-pp-size 4 \
      --actor-tp-size 1 --actor-pp-size 1 --actor-cp-size 1 \
      --ray-port 6789 \
...
mpirun -v --allow-run-as-root \
      ...
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/rule-only/qwen3vl_dense_grpo.sh $PLACE_CFG_FOLDER sampler >$LOG_DIR/sampler$PART.log 2>&1 &

mpirun -v --allow-run-as-root \
      --bind-to none --map-by slot --hostfile $PLACE_CFG_FOLDER/critic.hostfile \
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/rule-only/qwen3vl_dense_grpo.sh $PLACE_CFG_FOLDER critic >$LOG_DIR/critic$PART.log 2>&1 &

mpirun -v --allow-run-as-root \
      --bind-to none --map-by slot --hostfile $PLACE_CFG_FOLDER/actor.hostfile \
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/rule-only/qwen3vl_dense_grpo.sh $PLACE_CFG_FOLDER actor >$LOG_DIR/actor$PART.log 2>&1 &
