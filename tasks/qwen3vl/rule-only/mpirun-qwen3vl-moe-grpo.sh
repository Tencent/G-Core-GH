...
LOG_DIR="$PWD/log/qwen3vl-moe-geometry3k"
PART=1
...
# 235B
# --actor-tp-size 4 --actor-pp-size 8 --actor-cp-size 1 --actor-ep-size 16 \
...
mpirun -v --allow-run-as-root \
      ...
      --actor-tp-size 2 --actor-pp-size 1 --actor-cp-size 1 --actor-ep-size 8 \
      --ray-port 6789 \
...
mpirun -v --allow-run-as-root \
      ...
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/rule-only/qwen3vl_moe_grpo.sh $PLACE_CFG_FOLDER sampler $USE_GSPO >$LOG_DIR/sampler$PART.log 2>&1 &

mpirun -v --allow-run-as-root \
      --bind-to none --map-by slot --hostfile $PLACE_CFG_FOLDER/critic.hostfile \
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/rule-only/qwen3vl_moe_grpo.sh $PLACE_CFG_FOLDER critic $USE_GSPO >$LOG_DIR/critic$PART.log 2>&1 &

mpirun -v --allow-run-as-root \
      --bind-to none --map-by slot --hostfile $PLACE_CFG_FOLDER/actor.hostfile \
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/rule-only/qwen3vl_moe_grpo.sh $PLACE_CFG_FOLDER actor $USE_GSPO >$LOG_DIR/actor$PART.log 2>&1 &
