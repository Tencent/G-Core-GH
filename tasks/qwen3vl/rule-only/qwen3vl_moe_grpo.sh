...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  readonly SAVE_CHECKPOINT_DIR="${PWD}/ckpt_qwen3vl_30b_moe_grpo_actor_save"
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  EXTRA_ARGS+=" --wandb-exp-name grpo/rule-only/30b-moe/geometry3k/Instruct/baseline "
  EXTRA_ARGS+=" --wandb-save-dir $WANDB_DIR "
elif [ "$PPO_ROLE" = "critic" ]; then
  EXTRA_ARGS+=" --ppo-mm-rule-type geometry3k-v2 "
fi
