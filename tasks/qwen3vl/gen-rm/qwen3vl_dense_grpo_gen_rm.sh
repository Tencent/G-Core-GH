...
GEN_RM_NNODES=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-nnodes`
GEN_RM_MASTER_ADDR=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-master-addr`
GEN_RM_SVR_IPS=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-svr-ips`
GEN_RM_SVR_PORTS=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-svr-ports`
GEN_RM_DIST_INIT_ADDRS=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-dist-init-addrs`
GEN_RM_TP_SIZE=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-tp-size`
GEN_RM_PP_SIZE=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn gen-rm-pp-size`
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  readonly SAVE_CHECKPOINT_DIR="${PWD}/ckpt_qwen3vl_4b_gen_rm_actor_save"
  ...
  # readonly LOAD_CHECKPOINT_DIR="$MYWD/hf-hub/Qwen/Qwen3-VL-235B-A22B-Instruct"
  readonly LOAD_CHECKPOINT_DIR="$MYWD/hf-hub/Qwen/Qwen3-VL-8B-Instruct"
  readonly REF_LOAD_CHECKPOINT_DIR=$LOAD_CHECKPOINT_DIR
  readonly SAVE_CHECKPOINT_DIR="none"
...
readonly DATASET_META="/tmp/pmc_vqa_grpo_dataset_v3.json"
...
# RM_TOKENIZER_MODELS="$MYWD/hf-hub/Qwen/Qwen3-VL-235B-A22B-Instruct"
RM_TOKENIZER_MODELS="$MYWD/hf-hub/Qwen/Qwen3-VL-8B-Instruct"
...
if [ "$PPO_ROLE" = "gen-rm" ]; then
  # readonly MODEL_YAML="gpatch/model_yamls/qwen3vl-235b-a22b-moe.yaml"
  readonly MODEL_YAML="gpatch/model_yamls/qwen3vl-8b.yaml"
...
RL_ARGS="$GEN_ARGS
    ...
    --sglang-gen-rm-enable-weights-cpu-backup \
"
...
if [ "$PPO_ROLE" = "actor" ]; then
  RUN_PY='./tasks/qwen3vl/train_qwen3vl_ppo_actor.py'
  ...
  RUN_PY='./tasks/qwen3vl/train_qwen3vl_ppo_gen_rm.py'
elif [ "$PPO_ROLE" = "sampler" ]; then
  ...
  RUN_PY='./tasks/qwen3vl/train_qwen3vl_ppo_sampler.py'
else
  echo "no ${PPO_ROLE}"
  exit 0
fi
...
if [ "$PPO_ROLE" = "actor" ]; then
  EXTRA_ARGS+=""
  EXTRA_ARGS+=" --wandb-project qwen3vl-base "
  EXTRA_ARGS+=" --wandb-exp-name gen-rm/4b-dense/pmc-vqa/baseline "
  EXTRA_ARGS+=" --wandb-save-dir $WANDB_DIR "
elif [ "$PPO_ROLE" = "gen-rm" ]; then
  EXTRA_ARGS+=" --ppo-mm-rule-type pmc_vqa "
fi
