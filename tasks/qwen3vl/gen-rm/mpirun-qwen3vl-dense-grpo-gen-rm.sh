...
LOG_DIR="$PWD/log/qwen3vl-dense-gen-rm"
PART=1
...
mpirun -v --allow-run-as-root \
      ...
      --gen-rm-tp-size 2 --gen-rm-pp-size 1 \
      --actor-tp-size 2 --actor-pp-size 2 --actor-cp-size 1 \
      --ray-port 6789
...
mpirun -v --allow-run-as-root \
      ...
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/gen-rm/qwen3vl_dense_grpo_gen_rm.sh $PLACE_CFG_FOLDER sampler >$LOG_DIR/sampler$PART.log 2>&1 &

mpirun -v --allow-run-as-root \
      --bind-to none --map-by slot --hostfile $PLACE_CFG_FOLDER/gen-rm.hostfile \
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/gen-rm/qwen3vl_dense_grpo_gen_rm.sh $PLACE_CFG_FOLDER gen-rm >$LOG_DIR/gen-rm$PART.log 2>&1 &

mpirun -v --allow-run-as-root \
      --bind-to none --map-by slot --hostfile $PLACE_CFG_FOLDER/actor.hostfile \
      --mca btl_tcp_if_include bond1 --mca oob_tcp_if_include bond1 --mca routed direct \
      -x PATH -x LIBRARY_PATH -x LD_LIBRARY_PATH \
      bash tasks/qwen3vl/gen-rm/qwen3vl_dense_grpo_gen_rm.sh $PLACE_CFG_FOLDER actor >$LOG_DIR/actor$PART.log 2>&1 &
