...
# readonly MODEL_YAML= "$MYWD/gpatch/model_yamls/glm4.5v.yaml"


readonly HF_HUB_DIR="$MYWD/hf-hub/zai-org/GLM-4.1V-9B-Thinking"
readonly MODEL_YAML="$MYWD/gpatch/model_yamls/glm4.1v.yaml"
export IMAGE_TOKEN_ID=151343
...
CRITIC_EP_SIZE=`python tools/auto_place.py --fn get --config-folder $TOPO_CONFIG_FOLDER --get-fn critic-ep-size`
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  RUN_PY='./tasks/glm4v/grpo/train_glm4vl_ppo_critic.py'
elif [ "$PPO_ROLE" = "sampler" ]; then
