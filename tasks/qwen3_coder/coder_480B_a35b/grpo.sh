...
readonly PPO_ROLE=$2
# 因为 mpirun 脚本里 sampler 最先拉起，一个节点做一次就够了
...
readonly WANDB_PROJECT="qwen3_coder"
readonly WANDB_EXP_NAME="grpo_train"
export WANDB_BASE_URL=
export WANDB_API_KEY=

DFS_PATH="/mnt/ceph-sh2/models/"

readonly ACTOR_TOKENIZER_MODEL="$DFS_PATH/Qwen3-Coder-480B-A35B-Instruct/"
readonly RM_TOKENIZER_MODELS="$DFS_PATH/Qwen2.5-Math-RM-72B"
# temp
# readonly RM_TOKENIZER_MODELS="$DFS_PATH/Qwen3-Coder-480B-A35B-Instruct/"
...
if [ "$PPO_ROLE" = "actor" ]; then
  ...
  # 对于 actor 角色，LOAD_CHECKPOINT_DIR / LOAD_CHECKPOINT_DIR 填入 mlm ckpt 
  readonly REF_LOAD_CHECKPOINT_DIR="${PWD}/qwen3_coder_480b_a35b_moe_mlm"
  readonly LOAD_CHECKPOINT_DIR="${PWD}/qwen3_coder_480b_a35b_moe_mlm"
  readonly SAVE_CHECKPOINT_DIR="${PWD}/qwen3_coder_480b_a35b_moe_mlm_grpo_save"
  ...
  readonly MODEL_YAML="gpatch/model_yamls/qwen3-coder-480b-a35b-instruct.yaml"
  # 对于 sampler 角色，LOAD_CHECKPOINT_DIR 填入 hf ckpt 
  readonly LOAD_CHECKPOINT_DIR="$DFS_PATH/Qwen3-Coder-480B-A35B-Instruct/"
  readonly REF_LOAD_CHECKPOINT_DIR=$LOAD_CHECKPOINT_DIR
  readonly SAVE_CHECKPOINT_DIR="none"
  readonly TOKENIZER_MODEL=$ACTOR_TOKENIZER_MODEL
...
RL_ARGS=" \
    ...
    --update-weight-max-size-mb 256 \
    --sampler-gpu-memory-utilization 0.65 \
...
if [ "$PPO_ROLE" = "sampler" ] || [ "$PPO_ROLE" = "critic" ];  then
  RL_ARGS="$RL_ARGS
      --no-fused-kernel \
  "
fi
