...
class HermesToolParser(ToolParser):
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        for match in matches:
            try:
                # NOTE:
                # The model may generate JSON with raw control characters (e.g. newline) inside strings,
                # which is invalid in strict JSON and will raise:
                #   "Invalid control character at: line ..."
                # Use a non-strict JSON decoder to be more tolerant.
                function_call = json.JSONDecoder(strict=False).decode(match)
                ...
                logger.error(f"Problematic tool call content: {match}")
