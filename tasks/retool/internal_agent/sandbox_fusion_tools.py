...
from tasks.retool.internal_agent.base_tool import BaseTool
from tasks.retool.internal_agent.utils import _process_single_case
...
def _truncate(s: Any, max_chars: int = 2000) -> str:
    if s is None:
        return ""
...
class ExecutionWorker:
    def __init__(self, enable_global_rate_limit=True, rate_limit=10):
        self.rate_limit_worker = self._init_rate_limit(
            rate_limit
        ) if enable_global_rate_limit else None
...
class SandboxFusionTool(BaseTool):
    ...
    def execute_code(self, instance_id, code, timeout=30, language="python"):
        ...
        api_err = metadata.get("api_request_error")
        ...
        # Provide useful feedback to the agent (and to training logs) while avoiding huge outputs.
        if api_err:
            msg = f"[sandbox api_error] {_truncate(api_err)}"
            logger.warning(f"Sandbox API error instance_id={instance_id}: {msg}")
            return msg

        if compile_status and compile_status != "Finished":
            msg = f"[sandbox compile {compile_status}] {_truncate(compile_stderr)}"
            logger.info(
                f"Sandbox compile not finished instance_id={instance_id} api_status={api_status} compile_status={compile_status}"
            )
            return msg

        # "Finished" with non-zero exit code is still useful to return stderr.
        if run_status != "Finished" or (exit_code is not None and int(exit_code) != 0):
            diag = _truncate((stdout + "\n" + stderr).strip())
            msg = f"[sandbox run {run_status} exit_code={exit_code} duration={duration}] {diag}"
            logger.info(
                f"Sandbox run not clean instance_id={instance_id} api_status={api_status} run_status={run_status} exit_code={exit_code} duration={duration}"
            )
            return msg

        actual_output = (stdout + stderr).strip()
        actual_output = _truncate(actual_output, max_chars=8000)
        logger.debug(
            f"actual_output from sandbox fusion (truncated) instance_id={instance_id}: {actual_output}"
        )
